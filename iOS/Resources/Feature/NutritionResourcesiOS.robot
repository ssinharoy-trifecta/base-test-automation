...
${searchBarFieldiOS}    xpath=//XCUIElementTypeSearchField[@name="Search for food"]
${foodScannerIconiOS}    xpath=//XCUIElementTypeButton[@name="new scanner"]
...
${addFirstFoodItemBtniOS}    xpath=//XCUIElementTypeButton[@name="new add"][1]
...
Add Weight To The Weight Card iOS
    ...
    Wait Until Page Contains Element    ${keyboardDeleteBtniOS}    timeout=20
...
Log Food To Breakfast
    ...
    Wait Until Element Is Visible        ${searchBarFieldiOS}    timeout=20
    Input Value                          ${searchBarFieldiOS}    Avocado
    @{list}=    Create List              ${addFirstFoodItemBtniOS}
    ...                                  ${doneBtnNutritionKeyboardiOS}
    ...                                  ${doneAddingFoodBtniOS}
    FOR                                  ${item}    IN    @{list}
        Wait Until Element Is Visible    ${item}    timeout=20
        Click Element                    ${item}
    END
    Wait Until Page Contains Element     ${foodItemAvocadoiOS}    timeout=20

Log Food To Lunch
    Wait Until Element Is Visible        ${lunchBtniOS}    timeout=20
    Click Element                        ${lunchBtniOS}
    Wait Until Element Is Visible        ${searchBarFieldiOS}    timeout=20
    Input Value                          ${searchBarFieldiOS}    Pasta
    @{list}=    Create List              ${addFirstFoodItemBtniOS}
    ...                                  ${doneBtnNutritionKeyboardiOS}
    ...                                  ${doneAddingFoodBtniOS}
    FOR                                  ${item}    IN    @{list}
        ...
        Click Element                    ${item}
    END
    Wait Until Page Contains Element     ${foodItemPastaiOS}    timeout=20
